...
public class CreateDirectoryDialog extends DialogFragment implements OverwriteFileDialog.Overwritable {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		LinearLayout view = (LinearLayout) inflater.inflate(
				com.og.filemanager.R.layout.dialog_text_input, null);
		final EditText v = (EditText) view.findViewById(com.og.filemanager.R.id.foldername);
		v.setHint(com.og.filemanager.R.string.folder_name);
		...
		return new AlertDialog.Builder(getActivity())
				...
				.setTitle(com.og.filemanager.R.string.create_new_folder)
	...
	private void createFolder(final CharSequence text, Context c) {
		if (text.length() != 0) {
			...
			if (tbcreated.exists()) {
				...
				if (tbcreated.mkdirs())
					Toast.makeText(c, com.og.filemanager.R.string.create_dir_success, Toast.LENGTH_SHORT).show();
				else
					Toast.makeText(c, com.og.filemanager.R.string.create_dir_failure, Toast.LENGTH_SHORT).show();
