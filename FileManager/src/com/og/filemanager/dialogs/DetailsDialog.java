...
public class DetailsDialog extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		final View v = inflater.inflate(com.og.filemanager.R.layout.dialog_details, null);
		
		// Fill the views
		((TextView) v.findViewById(com.og.filemanager.R.id.details_type_value)).setText(f.isDirectory() ? com.og.filemanager.R.string.details_type_folder :
																		(f.isFile() ? com.og.filemanager.R.string.details_type_file : com.og.filemanager.R.string.details_type_other));
		
		mSizeView = (TextView) v.findViewById(com.og.filemanager.R.id.details_size_value);
		new SizeRefreshTask().execute();
		
		String perms = (f.canRead() ? "R" : "-") + (f.canWrite() ? "W" : "-") + (FileUtils.canExecute(f) ? "X" : "-");
		((TextView) v.findViewById(com.og.filemanager.R.id.details_permissions_value)).setText(perms);
		
		((TextView) v.findViewById(com.og.filemanager.R.id.details_hidden_value)).setText(f.isHidden() ? com.og.filemanager.R.string.details_yes : com.og.filemanager.R.string.details_no);
		
		((TextView) v.findViewById(com.og.filemanager.R.id.details_lastmodified_value)).setText(mFileHolder.getFormattedModificationDate(getActivity()));
	...
	private class SizeRefreshTask extends AsyncTask<Void, Void, String> {
		...
		protected void onPreExecute() {
			mSizeView.setText(com.og.filemanager.R.string.loading);
		}
