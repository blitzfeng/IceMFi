...
import com.og.filemanager.bookmarks.BookmarksProvider;
import com.og.filemanager.dialogs.DetailsDialog;
import com.og.filemanager.dialogs.MultiCompressDialog;
import com.og.filemanager.dialogs.MultiDeleteDialog;
import com.og.filemanager.dialogs.RenameDialog;
import com.og.filemanager.dialogs.SingleCompressDialog;
import com.og.filemanager.dialogs.SingleDeleteDialog;
...
public abstract class MenuUtils {
	...
	static public void fillContextMenu(FileHolder item, Menu m, int menuResource, MenuInflater mi, Context context){
		...
		if (file.isDirectory()) {
			m.removeItem(com.og.filemanager.R.id.menu_send);
		}
		
		// If selected item is a zip archive
		if (!FileUtils.checkIfZipArchive(file)) {
			m.removeItem(com.og.filemanager.R.id.menu_extract);
		} else {
			m.removeItem(com.og.filemanager.R.id.menu_compress);
		}
	...
	public static boolean handleMultipleSelectionAction(final FileListFragment navigator, MenuItem mItem, List<FileHolder> fItems, Context context) {
		...
		switch (mItem.getItemId()) {
			case com.og.filemanager.R.id.menu_send:
				...
				context.startActivity(Intent.createChooser(intent, context.getString(com.og.filemanager.R.string.send_chooser_title)));
				break;
			case com.og.filemanager.R.id.menu_delete:
				...
				break;
			case com.og.filemanager.R.id.menu_move:
				...
				break;
			case com.og.filemanager.R.id.menu_copy:
				...
				if(VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB)
					ActionbarRefreshHelper.activity_invalidateOptionsMenu(navigator.getActivity());
				break;
			case com.og.filemanager.R.id.menu_compress:
	...
	public static boolean handleSingleSelectionAction(final SimpleFileListFragment navigator, MenuItem mItem, FileHolder fItem, Context context){
		...
		switch (mItem.getItemId()) {
		case com.og.filemanager.R.id.menu_open:
			navigator.openInformingPathBar(fItem);
			return true;
			
		case com.og.filemanager.R.id.menu_create_shortcut:
		...
		case com.og.filemanager.R.id.menu_rename:
		...
		case com.og.filemanager.R.id.menu_details:
        ...
        case com.og.filemanager.R.id.menu_extract:
		...
		case com.og.filemanager.R.id.menu_bookmark:
			...
			if(!query.moveToFirst()){
				...
				Toast.makeText(context, com.og.filemanager.R.string.bookmark_added, Toast.LENGTH_SHORT).show();
			}
			else{
				Toast.makeText(context, com.og.filemanager.R.string.bookmark_already_exists, Toast.LENGTH_SHORT).show();
			}
			query.close();
			return true;

		case com.og.filemanager.R.id.menu_more:
    ...
    static private void createShortcut(FileHolder fileholder, Context context) {
		...
		Parcelable icon = Intent.ShortcutIconResource.fromContext(context.getApplicationContext(), com.og.filemanager.R.drawable.ic_launcher_shortcut);
	...
	private static void sendFile(FileHolder fHolder, Context context) {
		...
		i = Intent.createChooser(i, context.getString(com.og.filemanager.R.string.menu_send));
		
		try {
			context.startActivity(i);
		} catch (ActivityNotFoundException e) {
			Toast.makeText(context, com.og.filemanager.R.string.send_not_available, Toast.LENGTH_SHORT).show();
		}
	}
	...
	private static void showWarningDialog(final FileHolder holder, final Context context) {
		...
		View warningView = li.inflate(com.og.filemanager.R.layout.dialog_warning, null);
		final CheckBox showWarningAgain = (CheckBox)warningView.findViewById(com.og.filemanager.R.id.showagaincheckbox);
		
		showWarningAgain.setChecked(PreferenceActivity.getShowAllWarning(context));
		
		new AlertDialog.Builder(context).setView(warningView).setTitle(context.getString(com.og.filemanager.R.string.title_warning_some_may_not_work))
				.setMessage(context.getString(com.og.filemanager.R.string.warning_some_may_not_work))
