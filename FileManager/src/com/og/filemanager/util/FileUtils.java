...
//        }
	...
	public static File createUniqueCopyName(Context context, File path, String fileName) {
		...
		file = FileUtils.getFile(path, context.getString(com.og.filemanager.R.string.copied_file_name, fileName).concat(extension));
		...
		while (copyIndex < 500) {
			file = FileUtils.getFile(path, context.getString(com.og.filemanager.R.string.copied_file_name_2, copyIndex, fileName).concat(extension));
	...
	public static void openFile(FileHolder fileholder, Context c) {
		...
		try {
            ...
            if (activities.isEmpty() || (activities.size() == 1 && c.getApplicationInfo().packageName.equals(activities.get(0).activityInfo.packageName))){
                Toast.makeText(c, com.og.filemanager.R.string.application_not_available, Toast.LENGTH_SHORT).show();
                return;
            } else {
                c.startActivity(intent);
            }
		} catch (ActivityNotFoundException|SecurityException e) {
			Toast.makeText(c.getApplicationContext(), com.og.filemanager.R.string.application_not_available, Toast.LENGTH_SHORT).show();
		}
	}
